...
apiRouter.get('/websites', async (request: AuthRequest, response, next) => { try { const websites = await db.website.findMany({ where: { organizationId: request.auth!.organizationId, deletedAt: null }, include: { audits: { orderBy: { createdAt: 'desc' }, take: 1, include: { score: true } } }, orderBy: { createdAt: 'desc' } }); response.json({ success: true, data: websites }); } catch (error) { next(error); } });
...
apiRouter.get('/audits/:id/score', async (request: AuthRequest, response, next) => {
  try {
    ...
    response.json({ success: true, data: audit.score });
  } catch (error) {
    next(error);
  }
});
